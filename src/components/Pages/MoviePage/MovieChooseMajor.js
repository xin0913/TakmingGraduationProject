...
const MovieChooseMajor = () => {
    ...
    const profession = [{profession:'導演', index: 0}, {profession:'製片', index: 1}, {profession:'燈光', index: 2}, {profession:'錄音', index: 3}, {profession:'演員', index: 4}, {profession:'美術', index: 5}, {profession:'梳化', index: 6}, {profession:'編劇', index: 7}, {profession:'後期製作', index: 8}, {profession:'動畫', index: 9}, {profession:'攝影', index: 10}]
    ...
    const getAllProfile = async() => {
        const res = await fetch('http://192.192.140.209:9453/react/movie/profile/select')
    ...
    const jobValueChange = async(e) => {
        ...
        const res = await fetch('http://192.192.140.209:9453/react/movie/profile/select/professional',{
            method: 'POST',
            body: Request,
            credentials: 'omit'
        })
    ...
    const areaValueChange = async(e) => {
        ...
        const Request = new FormData()
        Request.append("where", e.target.value)
        const res = await fetch(`http://192.192.140.209:9453/react/movie/profile/select/area`,{
            method: 'POST',
            body: Request,
            credentials: 'omit'
        })
    ...
    // --------------------------------------------------------------
    // display CheckBox
    const movieProfessionCheckBox = (profession) => {
        return(
            <label className='MovieCheckBoxLabel' key={profession.index}>
    ...
    const moviePlaceCheckBox = (place) => {
        return(
            <label className='MovieCheckBoxLabel' key={place.index}>
    ...
    const movieSelectResults = (selectResults) => {
        return(
            <Link to={'/moviechoosemajordetail/'+selectResults.uid} key={selectResults.uid}>
                <div>
    ...
    // --------------------------------------------------------------
    return (
        <div className="MovieChooseMajorMainDiv">
            {/* <!--https://www.wfublog.com/2021/05/input-radio-checkbox-css.html--> */}
            <div>
                <div>
                    <div className="MovieSpecialSkill">專長</div>
                    <form >
                        <div className="MovieCheckBox" value={job} onChange={jobValueChange}>
                            {profession.map(movieProfessionCheckBox)}
                        </div>
                    </form>
                    <hr/>
                </div>
                <div>
                    <div className="MovieSpecialSkill">地區</div>
                    <form >
                        <div className="MovieCheckBox" value={area} onChange={areaValueChange}>
                            {place.map(moviePlaceCheckBox)}
                        </div>
                    </form>
                    <hr/>
                </div>
            ...
            <div className='all' onChange={window.location.reload}>
                {allProfile.map(movieSelectResults)}
            </div>
        </div>
    )
}

export default MovieChooseMajor