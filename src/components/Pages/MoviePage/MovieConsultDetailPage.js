...
const MovieConsultHistoryPage = ({Wallet}) => {
  ...
  const getConsultAnswer = async () => {
    ...
    const res = await fetch('http://192.192.140.209:9453/react/movie/consult/select/qid',{
      method: 'POST',
      body: Request,
      credentials: 'omit'
    })
  ...
  const getQuestion = async () => {
    const Request = new FormData()
    Request.append("where",params.qid)
    const res = await fetch('http://192.192.140.209:9453/react/movie/consult_list/select/qid',{
      method: 'POST',
      body: Request,
      credentials: 'omit'
    })
  ...
  const sendNewAnswer = () => {
    ...
    // alert('123')
    fetch('http://192.192.140.209:9453/react/movie/consult/add/finish',{
      method: 'POST',
      body: Request,
      credentials: 'omit'
    })
...
export default MovieConsultHistoryPage